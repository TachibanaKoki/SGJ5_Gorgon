Shader "Instanced/EnemyTexture" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_MainTex ("Albedo (RGB)", 2D) = "white" {}
		_SubTex("SubTex",2D) = "while"{}
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 subc  = tex2D(_SubTex, IN.uv_MainTex) * UNITY_ACCESS_INSTANCED_PROP(_Color);
			o.Albedo = 0;
			o.Albedo = c*(1 - _Fill) + subc*_Fill;
